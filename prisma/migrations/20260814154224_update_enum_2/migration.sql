/*
  ...
  - The values [REALEASE] on the enum `AlbumStatus` will be removed. If these variants are still used in the database, this will fail.
  - The values [REALEASE] on the enum `TrackStatus` will be removed. If these variants are still used in the database, this will fail.

*/
-- AlterEnum
BEGIN;
CREATE TYPE "AlbumStatus_new" AS ENUM ('PROCESSING', 'RELEASED', 'BLOCKED');
...
-- AlterEnum
BEGIN;
CREATE TYPE "TrackStatus_new" AS ENUM ('PROCESSING', 'RELEASED', 'BLOCKED');
